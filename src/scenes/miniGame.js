...
export default class extends Phaser.Scene {
  ...
  create () {
    ...
    // TEXT FOR HP
    this.hitPointsText = this.add.text(32, 60, this.hitPointsString + this.hitPoints)
    this.hitPointsText.visible = false

    // INTRO TEXT
    this.introText = this.add.text(146, 200, 'Collect the squids from the squidRain to feed your Narwal! \nBut be careful of the Orcas! \nClick START to start playing and use the arrow keys to move!')
  ...
  decreaseHp () {
    if (this.gameStarted && !this.finishedGame) {
      ...
      if (this.hitPoints <= 0) {
        this.failGame()
        this.gameStarted = false
        this.gameOverText = this.add.text(130, 300, 'Game Over! Restart to play again', {
          font: '40px Pacifico',
          fill: '#0C1B33'
        })
  ...
  exitText () {
    this.timesUp = this.add.text(130, 300, `Time's Up!! \nCurrent Score: ${this.score} \nCoins Received: ${this.score}`, {
      font: '40px Pacifico',
      fill: '#0C1B33'
    })
    this.restartButton = this.add.text(400, 250, 'RESTART')
  ...
  update () {
    ...
    if (this.cursors.left.isDown) {
      this.miniNarwal.setVelocity(-300, 0)
    } else if (this.cursors.right.isDown) {
      this.miniNarwal.setVelocity(300, 0)
    }
    // UP & DOWN
    if (this.cursors.up.isDown) {
      this.miniNarwal.setVelocity(0, -300)
